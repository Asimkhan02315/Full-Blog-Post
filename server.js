...
app.get("/me/:id", auth_token, (req, res) => {
    ...
    connection.query(
        `SELECT  users.id, users.name, posts.id, posts.title, posts.description, posts.slug, posts.created_at FROM crud INNER JOIN posts ON users.id = posts.author_id WHERE posts.author_id = '${id}' AND users.email = '${req.Useremail}' ORDER BY created_at DESC`,
        (err, result) => {
            if (err) {
                res.send(err);
            } else {
...
app.post("/posts", jsonparser, auth_token, (req, res) => {
    const postdata = {
    ...
    connection.query(
        ...
        (err, rows) => {
            ...
            if (SlugCount > 0) {
                // console.log("hggh");
                connection.query(
                    `INSERT INTO posts(title, description, slug, author_id) VALUES ('${
            postdata.title
            ...
            postdata.author_id
                ...
                // console.log("bye");
                connection.query(
                    `INSERT INTO posts(title, description, slug, author_id) VALUES ('${postdata.title}', '${postdata.description}', '${slug}', '${postdata.author_id}')`,
                    (err) => {
                        if (err) {
...
app.get("/posts/:slug", (req, res) => {
    ...
    connection.query(
        `SELECT posts.id, users.name, posts.title, posts.description, posts.created_at, posts.updated_at FROM crud INNER JOIN posts ON users.id = posts.author_id WHERE posts.slug = '${slug}'`,
        (err, resp) => {
            if (err) {
                res.send(err);
            } else {
...
app.put("/posts/:id", jsonparser, auth_token, (req, res) => {
    const updatedData = {
        title: req.body.title,
        description: req.body.description,
        updatedAt: new Date(),
 ...
 AS Id_Count FROM posts WHERE posts.id = '${id}' AND posts.author_id = '${req.Userid}'`,
        (err, rows) => {
            ...
            if (idCount) {
                connection.query(
                    `UPDATE posts SET title = '${updatedData.title}', description = '${updatedData.description}', updated_at = '${updatedData.updatedAt}' WHERE posts.id= '${id}' AND posts.author_id = '${req.Userid}'`,
                    (err, result) => {
                        if (err) {
                            res.send(err);
                        } else {
